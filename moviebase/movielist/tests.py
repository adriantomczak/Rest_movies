...
class MovieTestCase(APITestCase):
    ...
    def _create_fake_movie(self):
        ...
        self.new_movie = Movie.objects.create(**movie_data)
        for actor in actors:
            self.new_movie.actors.add(self._find_person_by_name(actor))
    ...
    def test_get_movie_detail(self):
        response = self.client.get(f"/movies/{self.new_movie.id}/", {}, format='json')
        self.assertEqual(response.status_code, 200)
        for field in ["title", "year", "description", "director", "actors"]:
            self.assertIn(field, response.data)

    def test_delete_movie(self):
        response = self.client.delete(f"/movies/{self.new_movie.id}/", {}, format='json')
        self.assertEqual(response.status_code, 204)
        movie_ids = [movie.id for movie in Movie.objects.all()]
        self.assertNotIn(self.new_movie.id, movie_ids)
    ...
    def test_update_movie(self):
        ...
        response = self.client.patch(f"/movies/{self.new_movie.id}/", movie_data, format='json')
        self.assertEqual(response.status_code, 200)
        movie_obj = Movie.objects.get(id=self.new_movie.id)
